...
class TestToTreeSequence:
    def test_basic(self):
        ...
        assert ts.sequence_length == tsg.total_sequence_length
    ...
    def test_record_provenance_false(self):
        tsg = make_two_contig_archive()
        ts = tsg.to_ts(record_provenance=False)
        assert ts.num_provenances == tsg.contig("chr1").num_provenances

    def test_record_provenance_true_adds_entry(self):
        tsg = make_two_contig_archive()
        ref_count = tsg.contig("chr1").num_provenances
        ts = tsg.to_ts(record_provenance=True)
    ...
    def test_is_vacant_bits_encoded_for_mixed_sample_sets(self):
        ...
        auto_samples = set(tsg.contig("chr1").samples())
        x_samples = set(tsg.contig("chrX").samples())
...
class TestFromTreeSequence:
    ...
    def test_roundtrip_contig_metadata(self):
        ...
        ts = tsg.to_ts()
        tsg2 = tsgroup.from_ts(ts)
        meta = tsg2.contig("chr1").metadata[tsgroup.CONTIG_METADATA_KEY]
    ...
    def test_record_provenance_kwarg(self):
        tsg = make_two_contig_archive()
        ts = tsg.to_ts()
        tsg2 = tsgroup.from_ts(ts, record_provenance=False)
        assert tsg2.num_contigs == 2
    ...
    def test_from_ts_record_provenance_false(self):
        ...
        ts = tsg.to_ts(record_provenance=False)
        tsg2 = tsgroup.from_ts(ts, record_provenance=False)
        assert tsg2.contig("chr1").num_provenances == 0
        assert tsg2.contig("chr2").num_provenances == 0

    def test_from_ts_record_provenance_true(self):
        tsg = make_two_contig_archive()
        ts = tsg.to_ts(record_provenance=False)
        tsg2 = tsgroup.from_ts(ts, record_provenance=True)
        assert tsg2.contig("chr1").num_provenances == 1
        assert tsg2.contig("chr2").num_provenances == 1

    def test_from_ts_record_provenance_true_entry_has_version(self):
        tsg = make_two_contig_archive()
        ts = tsg.to_ts(record_provenance=False)
        tsg2 = tsgroup.from_ts(ts, record_provenance=True)
        record = json.loads(tsg2.contig("chr1").tables.provenances[-1].record)
        assert record["software"]["name"] == "tsgroup"
        assert record["software"]["version"] == tsgroup.__version__
    ...
    def test_roundtrip_is_vacant_bits_reconstruct_samples(self):
        ...
        orig_samples = {key: set(tsg[key].samples()) for key in tsg.contigs}
...
class TestFromSlim:
    ...
    def test_basic(self):
        ...
        tsg = tsgroup.from_slim([ts1, ts2])
        assert tsg.num_contigs == 2
        assert tsg.contig("chr1").sequence_length == 1000
        assert tsg.contig("chr2").sequence_length == 2000

    def test_nodes_marked_shared(self):
        ts1 = self._make_slim_ts(0, 0, "chr1", "A", 1000)
        tsg = tsgroup.from_slim([ts1])
        ts = tsg.contig("chr1")
...
class TestFromTreeSequences:
    def test_basic(self):
        ...
        assert tsg.num_contigs == 2
        assert tsg.contig("20").sequence_length == 1000
        assert tsg.contig("21").sequence_length == 2000
