...
class TestTreeSequenceGroupConstruction:
    def test_basic(self):
        ...
        assert tsg.num_contigs == 2

    def test_empty(self):
        tsg = tsgroup.TreeSequenceGroup({})
        assert tsg.num_contigs == 0
...
class TestCaching:
    ...
    def test_not_nonglobal_sample_arg_when_shared(self):
        tsg = make_two_contig_archive(mark_shared=True)
        assert not tsg.is_nonglobal_sample_arg
    ...
    def test_nonglobal_sample_count(self):
        ...
        assert tsg.nonglobal_sample_node_count == 4
...
class TestAccess:
    ...
    def test_getitem(self):
        tsg = make_two_contig_archive()
        key = tsgroup.ContigKey(0, 0, "chr1", "A")
        assert tsg[key].sequence_length == 1000
...
class TestReindex:
    ...
    def test_reindex_custom_order(self):
        tsg = make_two_contig_archive()
        tsg2 = tsg.reindex(order=["chr2", "chr1"])
        keys = tsg2.contigs
    ...
    def test_reindex_updates_metadata(self):
        tsg = make_two_contig_archive()
        tsg2 = tsg.reindex(order=["chr2", "chr1"])
        meta0 = tsg2.contig("chr2").metadata["contig"]
        assert meta0["index"] == 0
        meta1 = tsg2.contig("chr1").metadata["contig"]
...
class TestSimplify:
    def test_simplify_reduces_nodes(self):
        ...
        for ts in tsg2.values():
            assert ts.num_nodes <= tsg.contig("chr1").num_nodes

    def test_simplify_preserves_num_samples(self):
        tsg = make_two_contig_archive(mark_shared=True, num_samples=4)
        tsg2 = tsg.simplify()
        for ts in tsg2.values():
    ...
    def test_simplify_preserves_is_shared_flags(self):
        tsg = make_two_contig_archive(mark_shared=True)
        tsg2 = tsg.simplify()
        assert not tsg2.is_nonglobal_sample_arg
    ...
    def test_simplify_consistent_sample_ids_across_contigs(self):
        ...
        samples_chr1 = set(tsg2.contig("chr1").samples())
        samples_chr2 = set(tsg2.contig("chr2").samples())
    ...
    def test_simplify_empty_group(self):
        tsg = tsgroup.TreeSequenceGroup({})
        tsg2 = tsg.simplify()
        assert tsg2.num_contigs == 0

    def test_simplify_by_samples(self):
        tsg = make_two_contig_archive(mark_shared=True, num_samples=4)
        global_samples = sorted(tsg.global_phased_node_ids & set(tsg.contig("chr1").samples()))
        tsg2 = tsg.simplify(samples=global_samples[:2])
        for ts in tsg2.values():
    ...
    def test_simplify_by_individuals(self):
        ...
        tsg = make_two_contig_archive(mark_shared=True, num_samples=4)
        tsg2 = tsg.simplify(individuals=[0, 1])  # keep 2 out of 2 individuals
        for ts in tsg2.values():
            assert ts.num_samples == 4  # 2 individuals × 2 haplotypes

    def test_simplify_by_individuals_subset(self):
        tsg = make_two_contig_archive(mark_shared=True, num_samples=4)
        tsg2 = tsg.simplify(individuals=[0])  # keep 1 out of 2 individuals
        for ts in tsg2.values():
    ...
    def test_simplify_by_individuals_on_partial_arg(self):
        ...
        tsg = make_autosomes_plus_x_archive(num_samples=4)
        assert tsg.is_nonglobal_sample_arg
        tsg2 = tsg.simplify(individuals=[0])
        assert tsg2.num_contigs == 3

    def test_simplify_samples_and_individuals_raises(self):
        tsg = make_two_contig_archive(mark_shared=True)
        with pytest.raises(ValueError, match="both"):
            tsg.simplify(samples=[0, 1], individuals=[0])
